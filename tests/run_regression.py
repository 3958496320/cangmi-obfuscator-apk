# -*- coding: utf-8 -*-
"""
回归测试：对 sample_input.lua 用多组 seed 混淆，再用 lua5.3 执行，
校验输出与原始脚本完全一致。

用法：
    python3 tests/run_regression.py [--lua lua5.3] [--seeds 30]
"""
...
def main() -> int:
    ...
    ap.add_argument("--lua", default="lua5.3")
    ap.add_argument("--seeds", type=int, default=30)
    ap.add_argument("--hashseed", type=int, default=0)
    args = ap.parse_args()

    sample = os.path.join(_HERE, "sample_input.lua")
    ...
    for seed in range(1, args.seeds + 1):
        ...
        # 写临时文件并执行
        tmp = f"/tmp/_regress_{seed}.lua"
        with open(tmp, "w", encoding="utf-8") as f:
            f.write(code)
        run = subprocess.run([args.lua, tmp], capture_output=True, text=True,
                             timeout=30)
        if run.returncode != 0:
            err = run.stderr.strip().splitlines()
            msg = err[-1] if err else "(no stderr)"
            fails.append((seed, f"RUN-ERR rc={run.returncode} prof={prof}", msg))
            continue
    ...
    print(f"\n=== {npass}/{args.seeds} PASS (PYTHONHASHSEED={args.hashseed}) ===")
